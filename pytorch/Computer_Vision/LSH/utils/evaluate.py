...
def pr_curve(query_code, retrieval_code, query_targets, retrieval_targets, device):
    ...
    for i in range(num_query):
        ...
        tmp = (hamm <= torch.arange(0, num_bit + 1).reshape(-1, 1).float().to(device)).float() # num_query<=(num_bit, 1)-->(num_bit, num_query)
        total = tmp.sum(dim=-1)
        total = total + (total == 0).float() * 0.1
        t = gnd * tmp # num_query * (num_bit, num_query)-->(num_bit, num_query)
        count = t.sum(dim=-1)
        p = count / total
        r = count / tsum
        P[i] = p
