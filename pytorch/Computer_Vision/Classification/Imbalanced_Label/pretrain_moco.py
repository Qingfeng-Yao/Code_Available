...
'''
ImageNet-LT和iNaturalist使用MoCo进行预训练
核心代码思想:
	(1)分布式训练
	(2)模型: moco.builder.MoCo
	(3)输入图像: 数据增强
		Moco: moco.loader.TwoCropsTransform
		MoCo v1:
		...
		MoCo v2:
            ...
            transforms.ToTensor()
            transforms.Normalize
		v2主要增加了RandomApply以及GaussianBlur
	(4)训练
		每个epoch开始调整学习率: adjust_learning_rate
		train函数

下载数据: kaggle competitions download -c imagenet-object-localization-challenge
[python pretrain_moco.py --dataset imagenet --data <data_path>][模型使用resnet50]
'''
