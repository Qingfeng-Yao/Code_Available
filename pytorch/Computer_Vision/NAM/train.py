...
'''
python=3.6.13
库: torch==1.4.0 torchvision==0.5.0 numpy==1.19.5 pillow==8.4.0 thop

参考: [https://github.com/Christian-lyc/NAM]

cifar100
    [python3 train.py --att_type NAM -sr --prefix nam]
    [best acc: 79.950]
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet', help='model architecture: ' +' | '.join(model_names) +
                        ' (default: resnet18)')
parser.add_argument('--att_type', type=str, choices=['BAM', 'CBAM', 'NAM'], default=None)
parser.add_argument('--sparsity_regularization', '-sr', dest='sr', action='store_true', help='train with channel sparsity regularization')
